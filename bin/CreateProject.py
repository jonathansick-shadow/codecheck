...
cmd = 'cpptestcli -bdf %s -data %s -resource %s -config "%s" -exclude %s -localsettings %s -report %s -showdetails -appconsole stdout -publish -nobuild &> %s' % (ceBdf, workspace, package, rules, ceExcludes, ceLocalSettings, cePackageReport, cePackageReportLog)
print "COMMAND: %s" %(cmd)
