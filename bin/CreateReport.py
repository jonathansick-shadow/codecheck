...
if parser.opts.python :
    ...
    cmd = 'cd %s; find . -name .tests -prune -o -name "*[^Lib].py" -exec pylint \{\} \\; &> %s' % (svnPackageDir, cePackageReportPythonLog )
    ...
    cmd = 'cd %s; rm -f %s; scons -c; scons setenv=1 CXX="cpptestscan g++" &> %s' % (svnPackageDir,ceBdf,cePackageReportBdfLog)
    ...
    cmd = 'cpptestcli -bdf %s -data %s -resource %s -config "%s" -exclude %s -localsettings %s -report %s -showdetails -appconsole stdout -publish -nobuild &> %s' % (ceBdf, workspace, package, rules, ceExcludes, ceLocalSettings, cePackageReport, cePackageReportCppLog)
